/* AI Car Finder - Bundled Application */
"use strict";
(() => {
  // src/voiceRecorder.ts
  var VoiceRecorder = class {
    ...
    initializeRecognition() {
      ...
      this.recognition.lang = "en-US,ur-PK";
...
${str(snapshot)}`);
  ...
  var OpenAIService = class {
    constructor(apiKey, customBackendUrl) {
      this.openai = null;
      this.customBackendUrl = null;
      this.apiKey = apiKey;
      if (customBackendUrl) {
    ...
    async convertTextToFilters(text) {
...
- "Suzuki Alto do hazar atharah Islamabad mein" \u2192 {"brand":"Suzuki","model":"Alto","year":2018,"color":null,"city":"Islamabad","price":null}`;
    ...
    async convertWithOpenAISDK(prompt) {
      if (!this.openai) {
    ...
    async convertWithCustomEndpoint(text, prompt) {
  ...
  var APP_CONSTANTS = {
    ...
    // Business Rules
    MAX_CAR_AGE: 50,
    MIN_CAR_YEAR: (/* @__PURE__ */ new Date()).getFullYear() - 50,
    MAX_CAR_YEAR: (/* @__PURE__ */ new Date()).getFullYear() + 1,
    MAX_MILEAGE: 1e6,
    // UI
    DEBOUNCE_TIME: 300,
    TOAST_DURATION: 5e3,
  ...
  var CarService = class {
    ...
    async getRecentCars(page = 1, limit2 = 20) {
      try {
        const url = `${this.baseUrl}${API_ENDPOINTS.CARS.BASE}?page=${page}&limit=${limit2}`;
        ...
        return {
          ...
          limit: rawData.pagination?.limit || limit2,
    ...
    async searchCars(filters, page = 1, limit2 = 20) {
      try {
  ...
  function getConfig() {
    ...
    const envKey = window.__ENV__?.VITE_OPENAI_KEY || window.process?.env?.OPENAI_KEY;
    const envUrl = window.__ENV__?.VITE_CUSTOM_BACKEND_URL || window.process?.env?.CUSTOM_BACKEND_URL;
    if (envKey) {
      return {
        openAIKey: envKey,
    ...
    try {
      ...
      if (storedKey) {
        return {
          openAIKey: storedKey,
          customBackendUrl: storedUrl || void 0
        };
      }
    } catch (e) {
    }
  ...
  var App = class {
    ...
    formatPrice(amount) {
      if (amount >= 1e7) {
        return `Rs. ${(amount / 1e7).toFixed(2)} Crore`;
      }
      if (amount >= 1e5) {
        return `Rs. ${(amount / 1e5).toFixed(2)} Lakh`;
      }
      return `Rs. ${amount.toLocaleString()}`;
    }
    updateDuration(state) {
      const seconds = Math.floor(state.duration / 1e3);
      const maxSeconds = Math.floor(state.maxDuration / 1e3);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register("/service-worker.js").then(() => console.log("Service Worker registered")).catch((err) => console.error("Service Worker registration failed:", err));
    }
    window.app = app;
  });
})();
