...
export function getConfig() {
    ...
    const envKey = window.__ENV__?.VITE_OPENAI_KEY ||
        window.process?.env?.OPENAI_KEY;
    const envUrl = window.__ENV__?.VITE_CUSTOM_BACKEND_URL ||
        window.process?.env?.CUSTOM_BACKEND_URL;
    if (envKey) {
        return {
            openAIKey: envKey,
